...
class UCT:
    ...
    def work(self, state, root):
        ...
        while time.time() - t0 < self.time_budget and iteration < self.iter_budget:
            ...
            iteration += 1
            print(f'\r__iteration: {iteration}__ | pid: {pid}', end='')
    ...
    def get_action(self, state):
        ...
        with multiprocessing.Pool(processes=self.num_workers) as pool:
            print(f'Running {self.num_workers} agent(s)!')
            forest = [pool.apply_async(self.work, (state, root)) for i in range(self.num_workers)]
            roots = [forest[i].get()[0] for i in range(self.num_workers)]
            iteration = np.sum([forest[i].get()[1] for i in range(self.num_workers)])
